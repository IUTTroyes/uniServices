...
<template>
  <div>
    ...
    <div>
      <PermissionGuard permission="isScolarite" :showFallback="true">
          <Message severity="info" icon="pi pi-info-circle" class="mt-2 mb-10">
            <div class="flex">
            <Input type="checkbox" class="mr-2" v-model="checked" @change="toggleValueMessage"/>
            <p class="font-bold">Afficher le message pour le personnel</p>
            </div>
        ...
        <template #fallback>
          <Message v-if="checked" severity="info" icon="pi pi-info-circle" class="mt-2 mb-10">
            Nous traitons actuellement un volume élevé de tickets. Merci de limiter vos ouvertures de tickets aux besoins essentiels afin de nous aider à réduire les délais de réponse.
          </Message>
        </template>
      </PermissionGuard>
    ...
    <div class="flex flex-col items-end gap-4 w-full"></div>
  </div>
</template>