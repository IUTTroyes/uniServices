...
const previAnneeEnseignantOriginal = ref(null);
...
const getPreviEnseignant = async () => {
  ...
  try {
    ...
    previAnneeEnseignantOriginal.value = JSON.parse(JSON.stringify(previAnneeEnseignant.value));
  } catch (error) {
    hasError.value = true;
    console.error('Erreur lors du chargement des prévisionnels :', error);
  } finally {
...
const getTypesHrs = async () => {
  try {
    typeHrs.value = await getPersonnelEnseignantTypesHrsService(selectedPersonnel.value.id, selectedAnneeUniv.value.id);
...
const getEnseignementsSemestre = async (semestreId) => {
  try {
    ...
    const enseignements = await getEnseignementsService(params, '/mini');
...
const addPrevi = async () => {
  try {
    const personnelIri = `/api/personnels/${selectedPersonnel.value.id}`;
...
const refreshTotals = () => {
  ...
  const prevs = previAnneeEnseignant.value[0];
  const totals = previAnneeEnseignant.value[1];
  ...
  totals['Total'] = (totals['CM'] || 0) + (totals['TD'] || 0) + (totals['TP'] || 0);
};
...
const updateEnseignementPrevi = (previId, enseignement) => {
  const previ = previAnneeEnseignant.value[0].find((previ) => previ.id === previId);
  if (previ && enseignement) {
    previ.idEnseignement = enseignement.id;
    previ.libelleEnseignement = enseignement.label;
  }
};
...
const saveRow = async (previId) => {
  try {
    ...
    if (previ.idEnseignement) {
      dataToUpdate.enseignement = `/api/scol_enseignements/${previ.idEnseignement}`;
    }
    ...
    showDanger('Erreur lors de la sauvegarde de la ligne', error);
    console.error('Erreur lors de la sauvegarde de la ligne:', error);
  }
};

const cancelRow = (previId) => {
  const originalRow = previAnneeEnseignantOriginal.value[0].find(p => p.id === previId);
  if (originalRow) {
    const index = previAnneeEnseignant.value[0].findIndex(p => p.id === previId);
    if (index !== -1) {
      previAnneeEnseignant.value[0][index] = JSON.parse(JSON.stringify(originalRow));
    }
  }
  editingRowId.value = null;
  refreshTotals();
...
const additionalRows = computed(() => [
  ...
  [
    ...
    { footer: previSemestreAnneeUniv.value[1].TotalCM, colspan: 1, class: '!bg-purple-400/20 !text-nowrap', unit: ' h' },
    { footer: previSemestreAnneeUniv.value[1].TotalTD, colspan: 1, class: '!bg-green-400/20 !text-nowrap', unit: ' h' },
    { footer: previSemestreAnneeUniv.value[1].TotalTP, colspan: 1, class: '!bg-amber-400/20 !text-nowrap', unit: ' h' },
...
const columnsForm = ref([
  { header: 'Matière/ressource/SAE', field: 'libelleEnseignement', sortable: true, colspan: 1, form: true, formType: 'select', formOptions: enseignementList, id: 'id', placeholder: 'Sélectionner un enseignement', formAction: (previId, type, valeur) => {updateEnseignementPrevi(previId, valeur)}, class: '!text-wrap !min-w-64' },
...
const additionalRowsForm = computed(() => [
  ...
  [
    ...
    { footer: previAnneeEnseignant.value?.[1]?.['CM'], colspan: 2, class: '!bg-purple-400/20 !text-nowrap', unit: ' h' },
    { footer: previAnneeEnseignant.value?.[1]?.['TD'], colspan: 2, class: '!bg-green-400/20 !text-nowrap', unit: ' h' },
    { footer: previAnneeEnseignant.value?.[1]?.['TP'], colspan: 2, class: '!bg-amber-400/20 !text-nowrap', unit: ' h' },
    { footer: previAnneeEnseignant.value?.[1]?.['Total'], colspan: 1, class: '!text-nowrap', unit: ' h' },
  ],
  ...
  [
    ...
    { footer: previAnneeEnseignant.value?.[2]?.['Service'], colspan: 2, rowspan: 2, class: '!text-center', unit: ' h' },
      ...
      <div v-if="previSemestreAnneeUniv[0].length > 0">
        ...
        <div v-else>
          ...
          <PrevisionnelTable
              ...
              :headerTitle="`${selectedPersonnel.display} | Prévisionnel de l'année ${selectedAnneeUniv?.libelle}`"
              :headerTitlecolspan="1"/>
        </div>
      </div>
