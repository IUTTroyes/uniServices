...
const getAnneeForPn = async (pnId) => {
  try {
    isLoadingPn.value = true;
    selectedPn.value = await getPnsDiplome(pnId, selectedAnneeUniversitaire.id);
    if (selectedPn.value && selectedPn.value.annees) {
      nodes.value = transformData(selectedPn.value.annees);
    }
  } catch (error) {
    console.error('Erreur lors du chargement de l\'année pour le PN:', error);
    hasError.value = true;
  } finally {
    isLoadingPn.value = false;
  }
