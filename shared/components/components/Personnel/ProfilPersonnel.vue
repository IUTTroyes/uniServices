...
import { ValidatedInput, validationRules, PermissionGuard } from "@components";
    ...
    <div class="flex flex-col gap-2 md:w-2/3 w-full p-4">
      ...
      <div>
        <h1 class="text-2xl font-bold mb-4 flex gap-4">
          ...
          <!-- Edit button only visible to the personnel themselves or users with edit permissions -->
          <Button
            v-permission="['isPersonnel', 'canEditPersonnelDetails']"
            severity="warn"
            rounded
            variant="outlined"
        ...
        <Message severity="info" class="mb-4" icon="pi pi-info-circle">
          <span v-if="userStore.isPersonnel">
            Ces informations ne sont visibles que de vous et de la direction. Merci de maintenir ces informations à jour pour faciliter la communication interne.
          </span>
          <span v-else>
        ...
        <div v-else class="flex md:flex-row flex-col gap-4 flex-wrap">
          ...
          <PermissionGuard :permission="['isPersonnel', 'canViewPersonnelDetails']" :showFallback="true">
            <div class="flex md:flex-row flex-col gap-4 flex-wrap w-full">
              <ul class="md:w-1/3 flex flex-col gap-2">
                <li><span class="font-bold">Adresse :</span> {{ formatAdresse(props.etudiantSco.etudiant.adresseEtudiante || {}) || 'Non renseigné' }}</li>
              </ul>
